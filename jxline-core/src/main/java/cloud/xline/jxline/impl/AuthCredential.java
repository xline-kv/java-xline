...
/**
 * AuthTokenInterceptor fills header with Auth token of any rpc calls and refreshes token if the rpc
 * results an invalid Auth token error.
 */
class AuthCredential extends CallCredentials {
    ...
    private void authenticate(MetadataApplier applier) {
        ...
        VertxAuthGrpc.AuthVertxStub authFutureStub =
                VertxAuthGrpc.newVertxStub(this.manager.getChannel());
        ...
        if (manager.builder().authHeaders() != null) {
            ...
            manager.builder()
                    .authHeaders()
                    .forEach((BiConsumer<Metadata.Key, Object>) metadata::put);
        ...
        if (!interceptorsChain.isEmpty()) {
            authFutureStub =
                    authFutureStub.withInterceptors(
        ...
        AuthenticateRequest request =
                AuthenticateRequest.newBuilder().setNameBytes(user).setPasswordBytes(pass).build();
