...
import cloud.xline.jxline.Watch;
...
public class WatchImpl extends Impl implements Watch {
    ...
    protected WatchImpl(ClientConnectionManager connectionManager) {
                ...
                MoreExecutors.listeningDecorator(
                        Executors.newScheduledThreadPool(
                                ...
                                r -> {
                                    ...
                                    t.setDaemon(true);
                                    t.setName("jxline-watch-" + t.getName());
                                    return t;
                                }));
    ...
    public Watcher watch(ByteSequence key, WatchOption option, Listener listener) {
        if (closed.get()) {
            throw new RuntimeException("cannot new closed watch client");
        }
    ...
    final class WatcherImpl implements Watcher {
        ...
        void resume() {
            ...
            if (started.compareAndSet(false, true)) {
                ...
                rstream =
                        Util.applyRequireLeader(option.withRequireLeader(), stub)
                                .watch(
                                        stream -> {
                                            wstream.set(stream);
                                            stream.write(
                                                    WatchRequest.newBuilder()
                                                            .setCreateRequest(builder)
                                                            .build());
                                        })
                                .handler(this::onNext)
                                .endHandler(this::onCompleted)
        ...
        public void close() {
            ...
            synchronized (WatchImpl.this.lock) {
                if (closed.compareAndSet(false, true)) {
                    if (wstream.get() != null) {
                        if (id != -1) {
                            final WatchCancelRequest watchCancelRequest =
                                    WatchCancelRequest.newBuilder().setWatchId(this.id).build();
        ...
        public void requestProgress() {
            if (!closed.get() && wstream.get() != null) {
                WatchProgressRequest watchProgressRequest =
                        WatchProgressRequest.newBuilder().build();
                wstream.get()
                        .write(
                                WatchRequest.newBuilder()
                                        .setProgressRequest(watchProgressRequest)
                                        .build());
            }
        }
        ...
        private void onNext(WatchResponse response) {
            ...
            if (response.getCreated()
                    ...
                    && (response.getCancelReason().contains("etcdserver: permission denied")
                            || response.getCancelReason()
                                    .contains("etcdserver: invalid auth token"))) {

                // potentially access token expired
                connectionManager().authCredential().refresh();
                handleError(
                        XlineException.toXlineException(
                                ErrorCode.CANCELLED, response.getCancelReason(), null),
                        true);
                ...
                if (response.getWatchId() == -1) {
                    listener.onError(
                            XlineException.toXlineException(
                                    ErrorCode.ABORTED,
                                    "etcd server failed to create watch id",
                                    null));
                    return;
                }
                ...
                if (option.isCreatedNotify()) {
                    listener.onNext(new cloud.xline.jxline.watch.WatchResponse(response));
                }
                ...
                XlineException error;

                if (response.getCompactRevision() != 0) {
                    error =
                            new XlineException(
                                    ExecuteError.newBuilder()
                                            .setRevisionCompacted(
                                                    Revisions.newBuilder()
                                                            .setCurrentRevision(
                                                                    response.getCompactRevision())
                                                            .build())
                                            .build());
                    ...
                    error =
                            XlineException.toXlineException(
                                    ErrorCode.FAILED_PRECONDITION, reason, null);
                }

                handleError(error, false);
            } else if (cloud.xline.jxline.watch.WatchResponse.isProgressNotify(response)) {
                listener.onNext(new cloud.xline.jxline.watch.WatchResponse(response));
                revision = Math.max(revision, response.getHeader().getRevision());
                ...
                //
                listener.onNext(new cloud.xline.jxline.watch.WatchResponse(response, namespace));
                revision = response.getHeader().getRevision();
            } else if (response.getEventsCount() > 0) {
                listener.onNext(new cloud.xline.jxline.watch.WatchResponse(response, namespace));
                revision =
                        response.getEvents(response.getEventsCount() - 1).getKv().getModRevision()
                                + 1;
            }
        }

        private void onCompleted(Void unused) {
            listener.onCompleted();
        }

        private void onError(Throwable t) {
            handleError(
                    XlineException.toXlineException(t), shouldReschedule(Status.fromThrowable(t)));
        }

        private void handleError(XlineException etcdException, boolean shouldReschedule) {
            // sync with close()
            synchronized (WatchImpl.this.lock) {
                if (isClosed()) {
                    return;
            ...
            Futures.addCallback(
                    ...
                    new FutureCallback<Object>() {
                        ...
                        public void onFailure(@NonNull Throwable t) {
                            logger().warn("scheduled resume failed", t);
                        }
